...
import {
  ...
  //OpenPortalAllocationUserUsage,
  //openportalAllocationUserUsageList,
} from 'waldur-js-client';

import { getAllPages } from '@waldur/core/api';
import { translate } from '@waldur/i18n';
import { getChartSpec, palette } from '@waldur/openportal-remote/details/constants';

import { Period, Usage } from './types';

// dummy data type
export type OpenPortalRemoteAllocationUserUsage = {
    node_usage?: number;
    month: number;
    year: number;
    allocation: string;
...
// dummy function
const openportalRemoteAllocationUserUsageList = (
  params: { query: { page: number; allocation: string } },
): Promise<{
  data: OpenPortalRemoteAllocationUserUsage[];
  count: number;
}> => {
...
const getDistinctUsers = (
  userUsages: OpenPortalRemoteAllocationUserUsage[],
): OpenPortalRemoteAllocationUserUsage[] => {
  const distinctUsers: OpenPortalRemoteAllocationUserUsage[] = [];
  const map = new Map();
...
export const loadCharts = async (
  ...
  const userUsages = await getAllPages((page) =>
    openportalRemoteAllocationUserUsageList({
