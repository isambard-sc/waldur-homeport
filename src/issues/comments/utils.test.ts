...
describe('JIRA markup formatter', () => {
  const test = (input, output) =>
    expect(formatJiraMarkup(input, [attachment])).toBe(output);
  ...
  it('creates a link to an external resource', () => {
    test(
      'See also: [http://jira.atlassian.com]',
  ...
  it('creates a message to not founded internal resource', () => {
    test('See also: [^test.jpg]', 'See also: Unable to find: test.jpg');
  });

  it('creates a named link to an external resource', () => {
    test(
      'See also: [Atlassian|http://jira.atlassian.com]',
