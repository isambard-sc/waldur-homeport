...
interface ProjectTemplateEditDialogProps {
  resolve: {
    initialValues: {
      uuid: string;
    };
...
// API functions
const fetchProjectTemplate = async (uuid: string) => {
  const data = await get(`/openportal-project-template/${uuid}/`);
  if (!data) {
    throw new Error(translate('Project template not found.'));
  }

  // Use the _data versions of these as they have been fully-fetched by the serializer
...
const updateProjectTemplate = async (formData: ProjectTemplateFormValues, currentCustomer: any) => {
  const payload = {
  ...
  return patch(`/openportal-project-template/${formData.uuid}/`, payload);
};

// Main component
export const ProjectTemplateEditDialog: React.FC<ProjectTemplateEditDialogProps> = ({ resolve }) => {
  const { showErrorResponse, showSuccess } = useNotify();
  const { closeDialog } = useModal();
  const currentCustomer = useSelector(getCustomer);
  const user = useSelector(getUser);

  const [projectTemplate, setProjectTemplate] = useState<ProjectTemplateFormValues | null>(null);
  ...
  const handleSubmit = useCallback(async (formValues: ProjectTemplateFormValues) => {
    try {
      await updateProjectTemplate(formValues, currentCustomer);
  ...
  useEffect(() => {
    const loadProjectTemplate = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await fetchProjectTemplate(resolve.initialValues.uuid);
        setProjectTemplate(data);
      } catch (err) {
        const error = err instanceof Error ? err : new Error(String(err));
        setError(error);
        // Don't call showErrorResponse here to avoid dependency issues
          ...
          >
            ...
            >
              <Field
                ...
                maxLength={FIELD_CONSTRAINTS.MAX_PROJECTCLASS_LENGTH}
                validate={composeValidators(
                  validateRequired,
                  validateMaxLength(FIELD_CONSTRAINTS.MAX_PROJECTCLASS_LENGTH)
                )}
                required
              />
            </FormGroup>
            ...
            >
              <Field
                ...
                maxLength={FIELD_CONSTRAINTS.MAX_PORTALIDENTIFIER_LENGTH}
                validate={composeValidators(
                  validateRequired,
                  validateMaxLength(FIELD_CONSTRAINTS.MAX_PORTALIDENTIFIER_LENGTH)
                )}
                required
              />
            </FormGroup>
            ...
            >
              <Field
                ...
                maxLength={FIELD_CONSTRAINTS.MAX_PROJECT_SHORTNAME_LENGTH}
                validate={composeValidators(
                  validateRequired,
                  validateMaxLength(FIELD_CONSTRAINTS.MAX_PROJECT_SHORTNAME_LENGTH)
                )}
