...
export const RemoteProjectNotesDialog: FC<Props> = ({ resolve }) => {
  ...
  const notes = ((row.award_details as any)?.notes ?? row.notes ?? []) as Note[];
  ...
  return (
    <ModalDialog
      title={translate('Notes — {name}', {
        name: row.current_project_name || row.uuid,
      })}
